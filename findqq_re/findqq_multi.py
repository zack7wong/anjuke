...
class FindQ:
            ...
            if new_content is not None:
                ...
                try:
                    ...
                    try:
                        ...
                        driver.save_screenshot(url_md5 + '.png')
                        print("driver.save_screenshot ok")
                        ...
                        try:
                            ...
                            print(e, 'insert md5 error')

                except Exception as e:
                    print(e, 'selenium错误')
                    try:
                        os.remove(png_name)
                        print('remove png_name ok')
                    except Exception as e:
                        print(e, 'os remove error')
                finally:
                    driver.quit()
                    ...
                    try:
                        if png_name:
                            print('png_name,size >>', png_name, int(os.path.getsize(png_name) / 1000))
                            try:
                                ...
                                for png in png_list:
                                    ...
                                    if text is not None and len(text) > 1:
                                        ...
                                        if '微信' in text:
                                            ...
                                            x = re.findall(
                                                '(?:微信|微信号)[\：\:\f\r\t\v]{1,4}[\u4e00-\u9fa5\-_a-zA-Z0-9]{6,20}[a-zA-Z_\-0-9]',
                                                text.strip())
                                            # print(x, '<<<<<<<< ocr-微信<<<<<<<<')
                                            vx_list.extend(x)
                                        if 'qq' in text:
    ...
    def find_in_content(self, content):
        ...
        if '微信' in content:  # 6-20个字母数字下划线或者减号，字母开头
            c = re.findall('(?:微信|微信号)[\：\:\f\r\t\v]{1,4}[\u4e00-\u9fa5\-_a-zA-Z0-9]{6,20}[a-zA-Z_\-0-9]',
                           content.strip())
        return a, b, c
